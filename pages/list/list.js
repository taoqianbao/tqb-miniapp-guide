var requests = require('../../services/request.js');
...
Page({
  ...
  onLoad: function (option) {
    console.log(option)   //URL获取参数   比如：../list?id=3, 通过option.id获取值
  },
  ...
  toDetailPage: function (e) {
    ...
    var bid = e.currentTarget.dataset.bid; //记录id [data-bid]
...
function requestData() {
  ...
  console.log(start)
  requests.requestSearchBook({ q: 'tetris+language:assembly', sort: 'stars', order: 'desc' }, (data) => {
    if (data.total_count == 0) {
      //没有记录
      _this.setData({ totalRecord: 0 });
    } else {
      _this.setData({
        pageData: _this.data.pageData.concat(data.items),
        pageIndex: start + 1,
